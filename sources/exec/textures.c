...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/11/30 18:23:04 by ssitchsa          #+#    #+#             */
/*   Updated: 2024/12/18 02:46:30 by ssitchsa         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (ray->side == 0)
		return ((ray->map_x - data->player.x + (1 - ray->step_x) / 2)
			/ ray->ray_dir_x);
	else
		return ((ray->map_y - data->player.y + (1 - ray->step_y) / 2)
			/ ray->ray_dir_y);
}
...
void	draw_column(t_data *data, int x, t_draw *draw, t_texture *texture)
{
	...
	while (y < draw->draw_end)
	{
		...
		tex_pos += step;
		color = texture->data[tex_y * texture->width + draw->tex_x];
