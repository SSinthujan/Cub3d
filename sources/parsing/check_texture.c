...
/*   Updated: 2024/12/18 02:42:28 by ssitchsa         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	get_texture(t_config *config, char *str, int nbr)
{
	if (nbr == 1)
	{
		if (config->north_texture[0] || !get_word(str, config->north_texture,
				PATH_MAX))
			return (1);
	}
	if (nbr == 2)
	{
		if (config->south_texture[0] || !get_word(str, config->south_texture,
				PATH_MAX))
			return (1);
	}
	if (nbr == 3)
	{
		if (config->east_texture[0] || !get_word(str, config->east_texture,
				PATH_MAX))
			return (1);
	}
	if (nbr == 4)
	{
		if (config->west_texture[0] || !get_word(str, config->west_texture,
				PATH_MAX))
			return (1);
	}
	return (0);
